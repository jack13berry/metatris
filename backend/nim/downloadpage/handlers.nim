proc serveOptionsDownloadPage(ctx:Ctx, rec:DownloadPage = nil) {.async.} =
  ctx.headers.add("Allow", "OPTIONS, GET, POST, PATCH")


proc serveGetDownloadPage(ctx:Ctx, rec:DownloadPage = nil) {.async.} =
  var rec = if rec == nil: DownloadPage() else: rec
  ctx.body = rec.trmdefault(ctx)
  ctx.contentType = ContentType.html


proc servePostDownloadPage(ctx:Ctx, rec:DownloadPage = nil) {.async.} =
  if ctx.owner == nil:
    ctx.errNoOwner()
    return
  ...
  let recid = ctx.db.insertId(
    sql"""insert into "recs_downloadpage" (owner, grp, "title") values (?,?,?)""",
...
proc servePatchDownloadPage(ctx:Ctx, rec:DownloadPage = nil) {.async.} =
  if ctx.owner == nil:
    ctx.errNoOwner()
    return
  ...
  let queryOk = ctx.db.tryExec(
    sql("update recs_downloadpage set " & updates), values
  )
