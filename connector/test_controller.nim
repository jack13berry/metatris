...
if init(app):
  ...
  font = ttf.openFont("fnt/Montserrat-Regular.ttf", 16)
  ...
  if not image.load(app.renderer, "img/metatris.png"):
    sdl.logCritical(sdl.LogCategoryError,
                    "Can't load image: %s",
                    img.getError())
  ...
  let numAxes = sdl.joystickNumAxes(joy)
  ...
  while not done:
    ...
    for i in 0..numAxes:
      text[i + 6] = &"Axis {$i}: {$sdl.joystickGetAxis(joy, i)}"
